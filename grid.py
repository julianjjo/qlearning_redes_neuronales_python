...
class Grid():
    ...
    def generate_grilla_vacia(self):
        ...
        self.grilla = np.empty([self.size_x, self.size_y], dtype=int)
        self.grilla.fill(0)
    ...
    def random_posicion_jugador(self):
        ...
        while True:
            ...
            if self.grilla[pos_x][pos_y] == 0:
                self.jugador.set_posicion_x(pos_x)
                self.jugador.set_posicion_y(pos_y)
                self.grilla[pos_x][pos_y] = self.jugador.get_value()
                break
    ...
    def random_posicion_tesoro(self):
        ...
        while True:
            ...
            if self.grilla[pos_x][pos_y] == 0:
                ...
                self.grilla[pos_x][pos_y] = self.tesoro.get_value()
                break
    ...
    def accion_aleatoria(self, pos_x, pos_y):
        ...
        before_post_x = pos_x
        before_post_y = pos_y
        last_pos_x = self.size_x - 1
        last_pos_y = self.size_y - 1
        if accion == 0:
            if pos_x > 0:
                pos_x -= 1
            else:
    ...
    def generar_camino(self, pos_x=0, pos_y=0):
        ...
        if self.grilla[pos_x][pos_y] == self.tesoro.get_value():
            return pos_x, pos_y
        else:
            if self.grilla[pos_x][pos_y] == 0:
                self.grilla[pos_x][pos_y] = 1
    ...
    def get_random_grid(self):
        ...
        jugador_new = Jugador()
        self.set_jugador(jugador_new)
        self.random_posicion_jugador()
        tesoro_new = Tesoro()
        self.set_tesoro(tesoro_new)
    ...
    def __str__(self):
        grillaStr = list(self.grilla)
        grillaStr["v" if x == 1 for x in grillaStr]
        grillaStr["j" if x == 2 for x in grillaStr]
        grillaStr["T" if x == 3 for x in grillaStr]
        grillaStr["f" if x == -1 for x in grillaStr]
        print(grillaStr)
        strGrilla = '\n'.join(
            '\t'.join(
                '%f' %x for x in y ) for y in grillaStr)
        return strGrilla
